...
|---|---|---|
| ~~**Firebase Storage was never provisioned**~~ | **RESOLVED IN CODE (2026-08-01).** Storage needs the paid Blaze plan, so the app stopped using it. The avatar is downscaled to 192x192 and stored as a base64 field on the profile document — ~6 KB against Firestore's 1 MiB limit. No console step, no upgrade. | — |
